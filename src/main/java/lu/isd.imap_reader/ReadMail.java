...
public class ReadMail {
    ...
    public void openImapSession() {
        ...
        props.setProperty("mail.imap.ssl.enable", sslEnabled ? "true" : "false");
    ...
    private void processSaveToFile (javax.mail.Message msg, String subject,
        ...
        Path pathName = null;
        try {
            pathName = Paths.get(targetFolder.toString(), sanitizeFilename(subject) + ".eml");
        } catch(Exception e) {
    ...
    private static String sanitizeFilename(String name) {
        logger.debug("Ready to sanitize: {}", name);
        // Remove non printable chars except blank space
        String unicode = StringEscapeUtils.unescapeHtml4(name);
        String noInvisibleChar = unicode.replaceAll("[\r\n\t]", "");
        String clean = noInvisibleChar.replaceAll("[:\\\\/*?|!#$%^<>\"]", "").trim();
        logger.debug("Sanitized: {}", clean);
        return clean;
    }
